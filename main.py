...
def append(transactions_file, excel_file):
    ...
    try:
        step7_excel.main(transactions_file, excel_file, excel_sheet)
    except Exception as e:
        print(f'Error appending transactions to Excel file: {e}')
        exit(7)

def main(secret_file, tokens_file, banks_file, agreement_file, link_file, accounts_file, transactions_file, excel_file, excel_sheet):
...
if __name__ == '__main__':
    if len(sys.argv) != 4:
        print('Usage: main.py <secret_file> <excel_file> <excel_sheet>')
        sys.exit(0)
    ...
    excel_sheet = sys.argv[3]

    main(secret_file, tokens_file, banks_file, agreement_file, link_file, accounts_file, transactions_file, excel_file, excel_sheet)
